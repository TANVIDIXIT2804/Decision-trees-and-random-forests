...
class RandomForestClassifier():
    ...
    def fit(self, X, y):
        ...
        newV = int(X.shape[1] ** 0.5)
        for i, estimator in enumerate(self.estimators):
            newX = X.sample(n = newV, replace = True, axis = 1, random_state = i)
            newY = y
    ...
    def plot(self):
        ...
        xx, yy = np.meshgrid(np.arange(x_min, x_max, new1), np.arange(y_min, y_max, new1))
        ...
        for i in range(self.nEstimators):
            ...
            #self.plotOneEstimator(ax,self.estimators[i], ax = ax)
...
class RandomForestRegressor():
    ...
    def fit(self, X, y):
        ...
        newV = int(X.shape[1] /2)
        for estimator in self.estimators:
            newX = X.sample(n = newV, replace = True, axis = 1)
            newY = y[newX.index]
            estimator.fit(newX,newY)
